...
exports.deriveStableTag = deriveStableTag;
 ...
 */
async function resolveTagCommit({ octokit, context }, tagName) {
 ...
 */
function deriveStableTag(prereleaseTagName) {
    const match = prereleaseTagName.match(/^(v\d+\.\d+\.\d+)-[0-9A-Za-z-]+\.\d+$/);
...
promoteToStable = false) {
    ...
    if (promoteToStable) {
        ...
        if (!derived) {
            throw new Error(`Cannot promote '${targetTagName}' to stable: expected a semver prerelease tag like v1.2.3-rc.2.`);
        }
    ...
    if (targetRelease.prerelease) {
        ...
        let targetCommitish;
        if (stableTagName) {
            const prereleaseCommit = await resolveTagCommit({ octokit, context, logger }, targetTagName);
            if (!prereleaseCommit) {
            ...
            const existingStableCommit = await resolveTagCommit({ octokit, context, logger }, stableTagName);
            if (existingStableCommit && existingStableCommit !== prereleaseCommit) {
                throw new Error(`Stable tag ${stableTagName} already exists and points at ${existingStableCommit}, ` +
    ...
    if (stableTagName) {
        throw new Error(`promote-to-stable was set but target ${targetTagName} is already a ` +
